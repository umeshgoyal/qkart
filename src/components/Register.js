...
class Register extends React.Component {
  ...
  performAPICall = async () => {
      ...
      return await fetch(config.endpoint+'/auth/register',{
        method:"POST",
        body: JSON.stringify({
      ...
      .then(response=>{
        ...
        if(this.validateResponse(false,response)){
          return response;
        }
      ...
      if(this.state.username.length>=6 && this.state.username.length<=32){
        if(this.state.password!==""){
          if(this.state.password.length>=6 && this.state.password.length<=32){
            ...
            message.error("Password should be more than 6 characters & less than 32 characters!");
            return false;
          }
        }
        else{
        ...
        message.error("Username should be more than 6 characters & less than 32 characters!");
        return false;
      }
    }
    else{
