...
class Search extends React.Component {
  ...
  search = (text) => {
    ...
    this.setState({ filteredProducts: displayProducts });  
  
    // if(text!==null){
    //   let searchText=text.toLowerCase();
    //   let item=[];
    //  item=this.products.filter((product) => {
    //     if(product.name.toLowerCase().includes(searchText) || product.category.toLowerCase().includes(searchText)){
    //      return product;
    //     }
    //  });
    // this.setState({
    ...
    return (
      <>
        ...
        <Header history={this.props.history}>
          ...
          <Input.Search placeholder="Search" onSearch={(value,event)=>{
            this.search(value)
          }} onPressEnter={(e)=>{
        ...
        <Row>
          {/* Display products */}
          <Col
            xs={{ span: 24 }} md={this.state.loggedIn? 18 : 24 }
          >
          ...
          {this.state.loggedIn && this.products.length && (
            ...
            >
              <div>
                ...
                <Cart history={this.props.history } token={localStorage.getItem('token')} ref={this.cartRef} products={this.products} checkout={false}/>
              </div>
            </Col>
          )}
        </Row>
