...
class Checkout extends React.Component {
  ...
  deleteAddress = async (addressId) => {
    ...
    if (this.validateResponse(errored, response, "Deleted an address")) {
  ...
  checkout = async () => {
    ...
    if (this.validateResponse(errored, response, "checkout")) {
        ...
        localStorage.setItem("balance",this.state.balance - this.cartRef.current.calculateTotal());
   ...
   */
  order = async () => {
    console.log(this.state.selectedAddressIndex);
    if(this.state.balance < this.cartRef.current.calculateTotal()){
      message.error("Not have sufficient balance!");
    }
    else if(this.state.addresses.length===0 || this.state.selectedAddressIndex===undefined){
  ...
  async componentDidMount() {
    ...
    if(localStorage.getItem("username") && localStorage.getItem("token")){
      ...
      if(this.cartRef.current.state.items.length===0){
        ...
        message.error('Please add items to the cart before checking out')
      }
    }
    else{
